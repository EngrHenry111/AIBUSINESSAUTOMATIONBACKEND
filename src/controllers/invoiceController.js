...
exports.createInvoice = async (req, res, next) => {
  try {
    ...
    // that was live when they were issued, not float with the market later.
    if (!body.currency) {
      const company = await Company.findById(req.companyId).select('defaultCurrency');
      body.currency = company?.defaultCurrency || 'NGN';
    }
    const { exchangeRate, ngnEquivalent } = await currencyFieldsFor(body.currency, Number(body.total) || 0);
    body.exchangeRate = exchangeRate;
    body.ngnEquivalent = ngnEquivalent;
...
<div class="footer">
  ...
  ${invoice.currency !== 'NGN' && invoice.exchangeRate ? `<div style="margin-top:8px">Exchange rate at invoice date: 1 ${esc(invoice.currency)} = ${esc(formatCurrency(invoice.exchangeRate, 'NGN'))}</div>` : ''}
...
function invoiceEmailHtml(invoice, company) {
  ...
  return `
    ...
    ${invoice.currency !== 'NGN' && invoice.exchangeRate ? `<p style="color:#94a3b8;font-size:11.5px;margin:0 0 12px;">Exchange rate at invoice date: 1 ${esc(invoice.currency)} = ${esc(formatCurrency(invoice.exchangeRate, 'NGN'))}</p>` : ''}
